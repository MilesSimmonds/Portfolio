...
class ClaimRestService extends RestService 
{
    ...
    private function getAllClaims()
    {
		...
		if (!$connection->connect_error)
		{
			...
			if ($result = $connection->query($query))
			{
				...
				{
					$this->CLAIM[] = new CLAIM($row["claimid"], $row["driverid"], $row["Clm_amount"], $row["Clm_freq"], $row["Oldclam"], $row["Clmflg"]);
				}
				$result->close();
			}
			$connection->close();
    ...
    private function getClaimById($claimid)
    {
		...
		if (!$connection->connect_error)
		{
			...
			$statement->bind_result($claimid, $driverid, $Clm_amount, $Clm_freq, $Oldclam, $Clmflg);
			if ($statement->fetch())
			{
