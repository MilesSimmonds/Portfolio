    ...
    const fetchData = async () => {
      try {
        ...
        const aggregatedDataMap = dataArray.reduce((map, row) => {
          ...
          const totalBabies = row.TotalBabies || 0; // Use TotalBabies from the query

          // Sum up TotalBabies for each region
          if (map.has(regionName)) {
            map.set(regionName, map.get(regionName) + totalBabies);
          } else {
            map.set(regionName, totalBabies);
          }
    ...
    <div ref={containerRef} className="map-chart-container" style={{ width: "100%", height: "100%" }}>
      ...
      {hoverData && (
        <div className="hover-data">
          ...
          <p><strong>Total Babies Born:</strong> {hoverData.value}</p>
        </div>
      )}
    </div>
  );
